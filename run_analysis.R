...
# read in test data, append subject ID and activity labels
test <- read.table("./test/X_test.txt")
test.subject <- read.table("./test/subject_test.txt")
test.acts <- read.table("./test/y_test.txt")
test$subject <- test.subject$V1
test$act <- as.character(test.acts$V1)

# read in test & train data, append subject ID and activity labels
train <- read.table("./train/X_train.txt")
train.subject <- read.table("./train/subject_train.txt")
train.acts <- read.table("./train/y_train.txt")
train$subject <- train.subject$V1
train$act <- as.character(train.acts$V1)

# bind together test and train data
all.dat <- rbind(test, train)

# create dataframe of activity ID to descriptive label
act.labels <- data.frame(act = c("1", "2", "3", "4", "5", "6"),
                         act.label = c("WALKING", "WALKING_UPSTAIRS", 
                                      "WALKING_DOWNSTAIRS", "SITTING", 
                                      "STANDING", "LAYING"))
act.labels$act <- as.character(act.labels$act)

# join the descriptive labels to the data set of summary device readings
all.dat <- left_join(all.dat, act.labels, by = "act") 

# create vector of labels for all variables in the data frame 
varnames <- c("tBodyAcc-mean()-X",
              "tBodyAcc-mean()-Y",
              "tBodyAcc-mean()-Z",
              "tBodyAcc-std()-X",
              "tBodyAcc-std()-Y",
...
# select indeces for all variable names related to std deviations
stdevs <- grep("std", varnames)

# create list of measures to be retained
